    ...
    handlers = {
        ...
        onSaveBook: function () {
            ...
            if (!$(views.checkBoxLow).is(':checked') && !$(views.checkBoxHigh).is(':checked')) {
                $(views.errorMessage).html("Please select Low res PDF, High res PDF, or both.");
                isValid = false;
            }

            if (($(views.hidSelectedPagesIntro).val().length == 0) && ($(views.hidSelectedPagesAdvanced).val().length == 0)) {
                $(views.errorMessage).html("Please select at least one page to include in your e-book.");
                isValid = false;
            }
