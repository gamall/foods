...
namespace Syngenta.Data.Helpers
{
    public class RepositoryFactories
    {
        private IDictionary<Type, Func<DbContext, object>> GetSyngentaFactories()
        {
            return new Dictionary<Type, Func<DbContext, object>>
                {
                   {typeof(IUserRepository), dbContext => new UserRepository(dbContext)},
                   {typeof(IUserBookRepository), dbContext => new UserBookRepository(dbContext)},
                   {typeof(IUserBookPageRepository), dbContext => new UserBookPageRepository(dbContext)},
                   {typeof(IFailedLoginAttemptRepository), dbContext => new FailedLoginAttemptRepository(dbContext)},
                   {typeof(IUserModificationLogRepository), dbContext => new UserModificationLogRepository(dbContext)}
                };
        }
        ...
        {
            _repositoryFactories = GetSyngentaFactories();
        }
