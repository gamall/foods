$(function () {
    ...
    handlers = {
        ...
        onSaveBook: function () {
            ...
            if (!$(views.checkBoxLow).is(':checked') && !$(views.checkBoxHigh).is(':checked')) {
                $(views.errorMessage).html("Por favor, seleccione la resolución para su PDF: alta, baja o los dos");
                isValid = false;
            }

            if (($(views.hidSelectedPagesIntro).val().length == 0) && ($(views.hidSelectedPagesAdvanced).val().length == 0)) {
                $(views.errorMessage).html("Por favor, seleccione al menos una página para incluir en su libro electrónico.");
                isValid = false;
            }
    ...
    config = {
        baseImageFolder: "/es/resources/images/",
        pageSize: 2,
